...
namespace MRK
{
    public class ThemeManager
    {
        ...
        private string GetThemeBackingStoreName(ThemeMetadata metadata)
        {
            ...
            return $"{storeDir}/{AppUtils.GenerateMD5Hash($"{metadata.Id}@{metadata.Version}")}";
        }
        ...
        private async Task<List<ThemeProperty>> GetBuiltInThemeProperties(string themeName)
        {
            ...
            // read built in theme
            var css = await AppUtils.ReadEmbeddedResource($"CSS.Themes.{themeName}.css");
        ...
        private async Task<List<ThemeMetadata>> GetBuiltInThemes()
        {
            ...
            const string themesPrefix = $"{AppUtils.ResourcesPrefix}.CSS.Themes";
            ...
            foreach (var name in resourceNames)
            {
                ...
                {
                    var css = await AppUtils.ReadEmbeddedResource(name, false);
